...
class DAC_newblock(nn.Module):
    def __init__(self,in_ch):
        ...
        self.path1 = nn.Sequential(
            nn.Conv2d(in_ch,ch[0],kernel_size=1,stride=1,padding=0,dilation=1,bias=True),
            nn.BatchNorm2d(ch[0]),
            nn.ReLU(inplace=True),
        )

        self.path2 = nn.Sequential(
            nn.Conv2d(in_ch, ch[1], kernel_size=1, stride=1, padding=0, dilation=1, bias=True),
            nn.BatchNorm2d(ch[1]),
            nn.ReLU(inplace=True),
            nn.Conv2d(ch[1], ch[1], kernel_size=3, stride=1, padding=1, dilation=1, bias=True),
            nn.BatchNorm2d(ch[1]),
            nn.ReLU(inplace=True),
        )

        self.path3 = nn.Sequential(
            nn.Conv2d(in_ch, ch[2], kernel_size=1, stride=1, padding=0, dilation=1, bias=True),
            nn.BatchNorm2d(ch[2]),
            nn.ReLU(inplace=True),
            nn.Conv2d(ch[2], ch[2], kernel_size=3, stride=1, padding=1, dilation=1, bias=True),
            nn.BatchNorm2d(ch[2]),
            nn.ReLU(inplace=True),
            nn.Conv2d(ch[2], ch[2], kernel_size=3, stride=1, padding=2, dilation=2, bias=True),
            nn.BatchNorm2d(ch[2]),
            nn.ReLU(inplace=True),
        )

        self.path4 = nn.Sequential(
            nn.Conv2d(in_ch, ch[3], kernel_size=1, stride=1, padding=0, dilation=1, bias=True),
            nn.BatchNorm2d(ch[3]),
            nn.ReLU(inplace=True),
            nn.Conv2d(ch[3], ch[3], kernel_size=3, stride=1, padding=1, dilation=1, bias=True),
            nn.BatchNorm2d(ch[3]),
            nn.ReLU(inplace=True),
            nn.Conv2d(ch[3], ch[3], kernel_size=3, stride=1, padding=2, dilation=2, bias=True),
            nn.BatchNorm2d(ch[3]),
            nn.ReLU(inplace=True),
            nn.Conv2d(ch[3], ch[3], kernel_size=3, stride=1, padding=3, dilation=3, bias=True),
            nn.BatchNorm2d(ch[3]),
            nn.ReLU(inplace=True),
        )
...
class AttU_Net(nn.Module):
    ...
    def __init__(self, img_ch=3, output_ch=1):
        ...
        self.DAC_newblock = DAC_newblock(filters[4])
